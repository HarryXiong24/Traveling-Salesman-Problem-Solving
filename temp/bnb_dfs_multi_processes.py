...
def bnb_task(
    ...
    best_path = []
    recursive(bound, weight, level, path)
    return (best_path_cost, best_path)


def branch_and_bound_multi_processes(
    city_count: int, distance_data: List[List[float]], processes_count: int = 1
) -> tuple[float, List[int]]:
    ...
    for i in range(1, city_count):
        initial_path = [0] + [-1] * (city_count - 1)
        initial_path[1] = i
        initial_tasks.append((distance_data, city_count, bound, 0, 1, initial_path))

    # 使用进程池执行搜索任务
    with Pool(processes_count) as pool:
        results = pool.starmap(bnb_task, initial_tasks)
